...
void main() {
	...
	for (int i = 0; i < container_num; i++) {
		...
		if (p_p_cont) {
			...
			cout << p_p_cont_iter->next() << "\nПри помощи метода to_string():\n" << *p_p_cont 
				...
				<< "\nОтсортируем данный контейнер:\n";
			p_p_cont->selection_sort();
			cout << *p_p_cont << "\nИспользуя метод pop() для данного контейнера, удалим оттуда все элементы, за исключением начального в статических контейнерах.\n";
			for (int j = 0; j < elems_num; j++)
				cout << p_p_cont->pop() << " ";
			cout << "\n\n";
		}
	...
	for (int i = 0; i < container_num; i++) {
		...
		if (deque_cont) {
			...
			cout << *deque_cont << "\nИспользуя метод pop_back() для данного контейнера, удалим из его конца все элементы, которые были добавлены.\n";
			for (int j = 0; j < elems_num; j++)
				cout << deque_cont->pop_back() << " ";
			cout << "\nИспользуя метод pop_front() для данного контейнера, удалим из его начала все элементы, которые были добавлены.\n";
			for (int j = 0; j < elems_num; j++)
